...
def decrypt_message_try_pads(ciphertext, pads):
    ...
    '''
    result= ciphertext.decrypt_message(pads[0])
    max_num=0
    wordlist=load_words(WORDLIST_FILENAME)
    for i in range(len(pads)):
        temp_num=0
        for word in ciphertext.decrypt_message(pads[i]).text.split():
            if is_word(wordlist,word):
                temp_num+=1
        if temp_num>=max_num:
            max_num=temp_num
            result=ciphertext.decrypt_message(pads[i])
    return result
...
def decode_story():
    ...
    pads=get_story_pads()
    return decrypt_message_try_pads(ciphertext,pads).text
